...
def main():
    set_seed(42)
    # === 获取项目根目录（InfantVision 的上级目录）===
    ROOT_DIR = os.path.dirname(os.path.abspath(__file__))

    # Configuration paths
    config_path = os.path.join(ROOT_DIR, 'config/hrnet-r90jt.yaml')
    model_path = os.path.join(ROOT_DIR, 'ckpt/hrnet-r90jt.pth')
    face_detector_path = os.path.join(ROOT_DIR, 'ckpt/face.pt')
    video_file = os.path.join(os.path.dirname(ROOT_DIR), 'sample/data/50cm3kg1.mp4')
